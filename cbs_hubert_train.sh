...
for model_name in CbsAttentionHubertCornModel CbsAttentionHubertOrdinalRegressionModel; do
    export MODEL="$model_name"
    
    target_name=""
    if [ "$model_name" = "CbsAttentionHubertCornModel" ]; then
        target_name="hubert_cbs_att_corn"
    else
        target_name="hubert_cbs_att_coral"
    fi
	...
	python3 cbs_hubert_train.py --config cbs_hubert.yaml
    done
done
