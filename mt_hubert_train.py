...
from mt_hubert_dataset import MtHubertDataset, data_processing
from mt_hubert_solver import LitMtHubert
import pandas as pd
import yaml
...
def main(args, config: dict):
    # 1) モデルとデータローダーを準備
    model = LitMtHubert(config)

    train_dataset = MtHubertDataset(path=config['train_path'], config=config)
    ...
    valid_dataset = MtHubertDataset(path=config['valid_path'], config=config)
    ...
    if best_path:
        best = LitMtHubert.load_from_checkpoint(best_path, config=config)
        best.eval()
        ...
        for batch in valid_loader:
            ...
            with torch.no_grad():
                pred_int, pred_nat = best.model.predict(huberts.to(best.device))
